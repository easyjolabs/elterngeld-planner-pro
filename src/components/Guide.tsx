...
const visaTypes: VisaType[] = [
  ...
  { id: 'turkey_insured', label: 'Turkish/Moroccan/Tunisian/Algerian – socially insured', category: 'special', status: 'eligible' },
  
  // WORK PERMITS - Eligible
  { id: 'niederlassungserlaubnis', label: 'Niederlassungserlaubnis (Settlement Permit)', category: 'work', status: 'eligible', paragraph: '§9' },
  { id: 'daueraufenthalt_eu', label: 'Daueraufenthalt-EU (EU Long-term Residence)', category: 'work', status: 'eligible', paragraph: '§9a' },
  ...
  { id: 'work_permit', label: 'Work Permit (Aufenthaltserlaubnis)', category: 'work', status: 'eligible', paragraph: '§7' },
  { id: 'beschaeftigungsduldung', label: 'Beschäftigungsduldung (Employment Toleration)', category: 'work', status: 'eligible', paragraph: '§60d' },
  
  // STUDY/TRAINING - Conditional
  { id: 'student', label: 'Student Visa', category: 'study', status: 'conditional', condition: 'Only if currently employed, in Elternzeit, or receiving ALG', paragraph: '§16b' },
  { id: 'qualification', label: 'Qualification Recognition', category: 'study', status: 'conditional', condition: 'Only if currently employed, in Elternzeit, or receiving ALG', paragraph: '§16d' },
  { id: 'job_seeker', label: 'Job Seeker Visa', category: 'study', status: 'conditional', condition: 'Only if currently employed, in Elternzeit, or receiving ALG', paragraph: '§20' },
  ...
  { id: 'humanitarian_war', label: 'Humanitarian (War in Home Country)', category: 'humanitarian', status: 'conditional', condition: 'Only if employed OR 15+ months in Germany', paragraph: '§23 Abs. 1' },
  { id: 'humanitarian_hardship', label: 'Humanitarian (Hardship Case)', category: 'humanitarian', status: 'conditional', condition: 'Only if employed OR 15+ months in Germany', paragraph: '§23a' },
  { id: 'humanitarian_protection', label: 'Subsidiary/Humanitarian Protection', category: 'humanitarian', status: 'conditional', condition: 'Only if employed OR 15+ months in Germany', paragraph: '§25 Abs. 3-5' },
  ...
  { id: 'voluntary', label: 'European Voluntary Service', category: 'other', status: 'not_eligible', paragraph: '§19e' },
  
  { id: 'other', label: "I don't know / Other", category: 'other', status: 'conditional', condition: 'Check with your local Elterngeldstelle' },
];
...
const flow: FlowMessage[] = [
  ...
  { type: 'bot', content: "Elterngeld replaces part of your income while you care for your newborn. The rules can get tricky, though." },
  { type: 'bot', content: "This **free guide** walks you through everything. And if you'd like, we can **prepare your official application** so you don't have to deal with the 23-page form." },
  { type: 'bot', content: "Here's what you'll learn:" },
  { type: 'component', component: 'introChecklist' },
  { type: 'bot', content: "Let's start by checking your eligibility. It depends on your **citizenship and residence permit**.", pause: true, pauseLabel: "Am I eligible?" },
  ...
  // CTA
  { type: 'bot', content: "Ready to apply? We can pre-fill your official application and guide you through the remaining steps." },
...
// ===========================================
const LightbulbIcon = () => (
  <svg className="w-4 h-4 inline-block mr-1 -mt-0.5" style={{ color: colors.text }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
  </svg>
);
        ...
        {bulletGroup.map((item, idx) => {
          ...
          return (
            <li key={idx} className="relative" style={{ listStyle: 'none' }}>
              <span className="absolute" style={{ left: '-0.75rem' }}>•</span>
  ...
  const processLine = (line: string, lineIndex: number): React.ReactNode => {
    ...
    const withLinks = linkParts.map((part, i) => {
      ...
      return boldParts.map((boldPart, j) => 
        j % 2 === 1 ? <strong key={`${i}-${j}`} className="font-semibold" style={{ color: colors.textDark }}>{boldPart}</strong> : boldPart
      );
    });
    return <>{withLinks}</>;
  };
...
const SliderInputComponent: React.FC<SliderInputProps & { label?: string }> = ({ value, onChange, onConfirm, label }) => {
  ...
  return (
    ...
    >
      {/* Value Display */}
      <div className="mb-4">
        <span className="text-[12px]" style={{ color: colors.text }}>{label || 'Monthly net income'}</span>
        <div className="mt-1">
          <span className="text-[14px] mr-1" style={{ color: colors.text }}>€</span>
          <span className="text-2xl font-bold" style={{ color: colors.textDark }}>{value.toLocaleString()}</span>
        </div>
      </div>
      ...
      >
        {/* Tick Marks */}
        <div className="absolute top-1/2 left-0 right-0 flex justify-between px-0" style={{ transform: 'translateY(-50%)' }}>
        ...
        {/* Track Background */}
        <div className="absolute top-1/2 left-0 right-0 h-0.5 rounded-full -translate-y-1/2" style={{ backgroundColor: colors.textDark }} />
  ...
  return (
    <div className="flex items-center justify-between flex-1">
      ...
      <div className="flex items-center gap-1">
        ...
        <span style={{ color: colors.textDark, fontSize: '13px', fontWeight: 600, minWidth: '50px', textAlign: 'center' }}>
    ...
    return (
      <div className="py-4 space-y-2">
        {categoryOptions.map(cat => (
          ...
          >
            <span className="text-[14px] font-medium" style={{ color: colors.textDark }}>{cat.label}</span>
            {cat.sub && <p className="text-[12px] mt-0.5" style={{ color: colors.text }}>{cat.sub}</p>}
          </button>
        ))}
      </div>
    );
      ...
      <div className="space-y-2">
        {categoryVisas.map(visa => (
          <button
            ...
            style={{ backgroundColor: colors.white, border: `1.5px solid ${colors.border}` }}
          >
            <span className="text-[14px] font-medium" style={{ color: colors.textDark }}>{visa.label}</span>
            {visa.paragraph && <span className="text-[12px] ml-1.5" style={{ color: colors.text }}>({visa.paragraph})</span>}
          </button>
        ))}
      </div>
    </div>
...
const InfoBox: React.FC<InfoBoxProps> = React.memo(({ title, content, onOpenChat, noMargin, defaultOpen = false, isLast = false }) => {
  const [isOpen, setIsOpen] = React.useState(defaultOpen);
  
  return (
    <div style={{ borderTop: `1px solid ${colors.border}`, borderBottom: isLast ? `1px solid ${colors.border}` : 'none' }}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[14px]" style={{ color: colors.textDark }}>{title}</span>
        </div>
        <svg 
...
const ElterngeldGuide: React.FC<ElterngeldGuideProps> = ({ onOpenChat }) => {
  ...
  const [stepHistory, setStepHistory] = useState<Array<{step: number, messagesLength: number, savedShowInput: FlowMessage | null}>>([]);
  ...
  const streamingRef = useRef<{ words: string[], index: number, messageIndex: number, callback: (() => void) | null }>({ words: [], index: 0, messageIndex: -1, callback: null });
  ...
  const chatStreamingRef = useRef<{ words: string[], index: number, messageIndex: number }>({ words: [], index: 0, messageIndex: -1 });
  ...
  const cycleType = useCallback((monthIndex: number, person: 'you' | 'partner', e?: React.MouseEvent) => {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    ...
    setPlannerData(prev => {
      ...
      const cycle: Array<'none' | 'basis' | 'plus' | 'bonus'> = data.applicationType === 'single' 
        ? ['none', 'basis', 'plus'] 
        : ['none', 'basis', 'plus', 'bonus'];
  ...
  const jumpToCta = () => {
    ...
    const presetData = Array(32).fill(null).map((_, i) => ({
      you: i < 12 ? 'basis' : 'none',
    ...
    setMessages([
      { type: 'bot', content: "The hardest part is done – you've figured out your Elterngeld strategy! Now just add your personal details and we'll generate your official application." },
      { type: 'component', component: 'ctaCard' }
    ]);
    messagesLengthRef.current = 2;
    setData({ applicationType: 'couple', dueDate: '2025-06-15' });
  ...
  const plannerValidation = React.useMemo(() => {
    ...
    const getPrematureExtra = () => {
      switch (data.premature) {
        case '6weeks': return 1;
        case '8weeks': return 2;
        case '12weeks': return 3;
        case '16weeks': return 4;
        default: return 0;
      }
    };
    ...
    if (totalBudget > maxBudget) {
      global.push(`You have exceeded the ${maxBudget}-month limit (currently ${totalBudget}). Both parents share a total budget of ${maxBudget} Basis months.`);
    }
    ...
    if (isCouple && !hasMultiples) {
      if (youBasis > 12) {
        ...
        if (firstBasisIndex >= 0) addRowError(firstBasisIndex, `You have exceeded 12 Basis months (currently ${youBasis}). Max 12 per parent.`);
      }
      if (partnerBasis > 12) {
        const firstBasisIndex = plannerVisibleData.findIndex(m => m.partner === 'basis');
        if (firstBasisIndex >= 0) addRowError(firstBasisIndex, `Partner has exceeded 12 Basis months (currently ${partnerBasis}). Max 12 per parent.`);
      }
    }
    ...
    if (isCouple && !hasMultiples) {
      const parallelBasisIndices = plannerVisibleData.map((m, i) => (m.you === 'basis' && m.partner === 'basis') ? i : -1).filter(i => i >= 0);
    ...
    if (isCouple) {
      const bonusIndices = plannerVisibleData.map((m, i) => (m.you === 'bonus' || m.partner === 'bonus') ? i : -1).filter(i => i >= 0);
      
      if (bonusIndices.length > 0) {
        // Check if not parallel
        const notParallel = plannerVisibleData.some(m => (m.you === 'bonus' && m.partner !== 'bonus') || (m.partner === 'bonus' && m.you !== 'bonus'));
        
        // Check if not consecutive
        let notConsecutive = false;
        for (let j = 1; j < bonusIndices.length; j++) {
          if (bonusIndices[j] !== bonusIndices[j-1] + 1) { notConsecutive = true; break; }
        }
    ...
    switch(key) {
      case 'citizenshipResponse':
        if (data.citizenship === 'eu') {
          ...
          streamMessage({ type: 'bot', content: "What type of **residence permit** do you have?", isQuestion: true, subtext: "Select from common types or search for yours." }, () => {
...
If your partner can't claim, you may qualify as a **single parent** and use all 14 months yourself.`
      ...
      case 'plannerIntro':
        const plannerText = data.applicationType === 'couple' 
  ...
  const handleInput = (value: string | number, displayValue?: string) => {
    ...
    if (field) {
      ...
      if (['visaType', 'isEmployed', 'humanitarianCondition', 'ineligibleChoice', 'continueAfterStelle'].includes(field)) {
        return;
      }
    }
    ...
    switch(key) {
      case 'visaResponse':
        ...
        if (!visa) {
          ...
          if (visa.id === 'other') {
            showIneligibleOptions("Please check with your local **Elterngeldstelle** to confirm your eligibility based on your specific visa type.");
            return;
          } else if (['student', 'qualification', 'job_seeker'].includes(visa.id)) {
            streamMessage({ type: 'bot', content: "Are you currently **employed** in Germany?", isQuestion: true, subtext: "With your visa type, you're only eligible if you're working, in Elternzeit, or receiving unemployment benefits (ALG)." }, () => {
              setShowInput({
                type: 'bot',
                input: 'buttons',
                field: 'isEmployed',
            ...
            streamMessage({ type: 'bot', content: "Which applies to you?", isQuestion: true, subtext: "With your visa type, you're eligible if you're employed OR have been in Germany for 15+ months." }, () => {
              setShowInput({
                type: 'bot',
                input: 'buttons',
      ...
      case 'conditionalVisaResponse':
        if (['student', 'qualification', 'job_seeker'].includes(currentData.visaType || '')) {
          if (currentData.isEmployed === 'no') {
            showIneligibleOptions("Unfortunately, with your visa type you need to be employed, in Elternzeit, or receiving ALG to be eligible.");
            return;
        ...
        } else if (['humanitarian_war', 'humanitarian_hardship', 'humanitarian_protection'].includes(currentData.visaType || '')) {
          if (currentData.humanitarianCondition === 'neither') {
            showIneligibleOptions("Unfortunately, with your visa type you'll need to either be employed or have been in Germany for **15+ months** to be eligible.");
            return;
          } else if (currentData.humanitarianCondition === 'employed') {
            response = { type: 'bot', content: "Since you're employed, you **likely qualify** for Elterngeld!" };
          } else {
            response = { type: 'bot', content: "Since you've been in Germany 15+ months, you **likely qualify** for Elterngeld!" };
          }
        }
        break;
      ...
      case 'ineligibleChoiceResponse':
        if (currentData.ineligibleChoice === 'continue' || currentData.continueAfterStelle === 'yes') {
          response = { type: 'bot', content: "Alright, let's continue – you can still use this tool to **plan** and see what you would receive." };
          ...
          streamMessage({ type: 'bot', content: "I've opened the **Elterngeldstelle finder** for you. They can give you personalized advice on your specific situation." }, () => {
            setTimeout(() => {
              streamMessage({ type: 'bot', content: "Want to continue with the planning tool anyway?", isQuestion: true }, () => {
                setShowInput({
                  type: 'bot',
                  input: 'buttons',
          ...
          streamMessage({ type: 'bot', content: "No problem! Feel free to come back anytime. Good luck with your application! " }, () => {
  ...
  const CategoryHeader = React.memo(({ label }: { label: string }) => (
    <div className="pt-6 pb-2">
      <span className="text-[12px] font-bold uppercase flex items-center gap-2" style={{ color: colors.text, letterSpacing: '0.12em' }}>
        {categoryIcons[label]}
        {label}
      </span>
    </div>
  ...
  const calculationCardContent = React.useMemo(() => {
    ...
    const renderAmount = (current: number, original: number, size: string = '13px', bold: boolean = false) => {
      ...
      return <span style={{ color: colors.textDark, fontSize: size, fontWeight }}>€{current.toLocaleString('de-DE')}</span>;
    };
    ...
    return (
      <div className="py-3">
              ...
              >
                ...
                {openTooltips.has(item.label.toLowerCase()) && (
                  <div 
                    ...
                    style={{ backgroundColor: 'rgba(255,255,255,0.5)', fontSize: '12px', color: colors.textDark, lineHeight: 1.4 }}
                  >
                    {item.label === 'BASIS' 
                ...
                {/* Total */}
                <div className="mt-1 pt-1 flex items-center justify-between" style={{ borderTop: '1px solid rgba(0,0,0,0.1)' }}>
                  <span style={{ color: colors.textDark, fontSize: '13px' }}>
                    {item.maxMonths} months
                  </span>
                  <div>
                    {totalChanged && (
                      <span style={{ color: colors.textDark, fontSize: '12px', opacity: 0.5, textDecoration: 'line-through', marginRight: '4px' }}>
        ...
        {workPartTime && (
          <div className="mt-1">
            ...
            <div className="flex items-center gap-6">
              ...
              {isCouple && (
                <IncomeStepper
                  value={partnerPartTimeIncome}
                  label="Partner"
                  onChange={setPartnerPartTimeIncome}
                />
              )}
            </div>
          </div>
        ...
        >
          <svg className="w-4 h-4 shrink-0 mt-0.5" style={{ color: colors.textDark }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
          </svg>
  ...
  const PlannerComponent = () => {
    ...
    const applyPreset = (preset: '12+2' | '7+7' | '12-solo' | '14-solo' | 'clear') => {
      ...
      if (preset === '12+2') {
        setPlannerData(Array(32).fill(null).map((_, i) => ({
          you: i < 12 ? 'basis' : 'none',
          partner: i >= 12 && i < 14 ? 'basis' : 'none'
        })));
      } else if (preset === '7+7') {
        ...
        setPlannerData(Array(32).fill(null).map(() => ({ you: 'none', partner: 'none' })));
      }
    };

    // Use parent-level calculations (no hooks inside this function)
    const visibleData = plannerVisibleData;
    const { youBasis, youPlus, youBonus, partnerBasis, partnerPlus, partnerBonus, youBudget, partnerBudget, totalBudget, maxBudget, totalMoney, globalErrors, rowErrors } = plannerValidation;
    ...
    const getCardStyle = (type: 'none' | 'basis' | 'plus' | 'bonus', person: 'you' | 'partner', monthIndex: number) => {
      const hasError = errorMonths.includes(monthIndex) && 
                       lastEditedCell?.month === monthIndex && 
                       lastEditedCell?.person === person;
    ...
    return (
      <div className="py-4">
        ...
        <div className="mb-3 flex items-stretch gap-2" style={{ minHeight: 60 }}>
          {bonusResetMessage ? (
            ...
            <div 
              ...
              style={{ backgroundColor: 'rgba(255, 189, 240, 0.5)' }}
            >
              <svg className="w-4 h-4 shrink-0" style={{ color: colors.textDark }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
            ...
            >
              ...
              <svg className="w-4 h-4 shrink-0" style={{ color: colors.text }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
            ...
            >
              <svg className="w-4 h-4 shrink-0" style={{ color: plannerColors.errorText }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            ...
            >
              <svg className="w-4 h-4 shrink-0" style={{ color: plannerColors.successText }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
              <span className="line-clamp-2" style={{ color: plannerColors.successText, fontSize: '13px', lineHeight: 1.4 }}>
          ...
          >
            <span className="text-xs font-bold" style={{ color: totalBudget > maxBudget ? plannerColors.errorText : colors.textDark }}>
          ...
          <button
            ...
            style={{ backgroundColor: plannerColors.tipBg, minHeight: 60 }}
          >
            <svg className="w-4 h-4" style={{ color: colors.textDark }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
        ...
        <div className="flex items-center mb-2" style={{ gap: 8 }}>
          <div style={{ width: 24 }}>
            <span className="text-xs font-medium" style={{ color: colors.text }}>#</span>
          </div>
          <div className="flex-1">
            <span className="text-xs font-medium" style={{ color: colors.text }}>You</span>
          </div>
          {isCouple && (
            <div className="flex-1">
              <span className="text-xs font-medium" style={{ color: colors.text }}>Partner</span>
            </div>
          )}
          <div style={{ width: 72, marginLeft: 12 }}>
            <span className="text-xs font-medium" style={{ color: colors.text }}>Sum</span>
          </div>
            ...
            >
              <div className="flex flex-col" style={{ gap: rowGap }}>
                {visibleData.map((month, i) => {
                  const youAmount = month.you === 'basis' ? myCalc.basis : month.you === 'plus' ? myCalc.plus : month.you === 'bonus' ? myCalc.bonus : 0;
                  const partnerAmount = isCouple 
                    ? (month.partner === 'basis' ? partnerCalc.basis : month.partner === 'plus' ? partnerCalc.plus : month.partner === 'bonus' ? partnerCalc.bonus : 0)
                    : 0;
                  ...
                  return (
                    <div key={i} className="flex items-center" style={{ gap: 8, height: rowHeight }}>
                      ...
                      >
                        {(() => {
                          const hasError = errorMonths.includes(i) && lastEditedCell?.month === i && lastEditedCell?.person === 'you';
                          ...
                          return (
                            <div className="flex items-center justify-between w-full px-2">
                              ...
                              <div className="flex items-center gap-1">
                                {hasError && (
                                  <svg className="w-3 h-3" style={{ color: plannerColors.errorText }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                  </svg>
                                )}
                      ...
                      {isCouple && (
                        ...
                        >
                          {(() => {
                            const hasError = errorMonths.includes(i) && lastEditedCell?.month === i && lastEditedCell?.person === 'partner';
                            const textColor = hasError ? plannerColors.errorText : colors.textDark;
                      ...
                      <div style={{ width: 72, marginLeft: 12 }} className="flex items-center gap-1">
                        {infoTooltip && (
                          <div className="relative group">
                            <svg className="w-3.5 h-3.5 cursor-help" style={{ color: colors.text }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
        ...
        <div className="flex justify-between items-center mt-1">
            ...
            {isCouple ? (
              <>
                <button
                  ...
                  style={{ backgroundColor: colors.white, color: colors.textDark, border: `1px solid ${colors.border}` }}
                >
                  12 + 2
                </button>
                <button
          ...
          {plannerEmailSaved && (
            <div className="flex items-center gap-1.5">
              ...
              >
                <svg className="w-2.5 h-2.5" style={{ color: colors.success }} fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
              </div>
              <span className="text-xs" style={{ color: colors.text }}>Saved!</span>
            </div>
          )}
        </div>
        ...
        {showPlannerSaveInput && !plannerEmailSaved && (
          ...
          >
            ...
            >
              ...
              >
                <svg className="w-6 h-6" style={{ color: colors.textDark }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z" />
                </svg>
              </div>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-[12px]" style={{ color: colors.text }}>or</span>
                <div className="flex-1 h-px" style={{ backgroundColor: colors.border }} />
              </div>
              ...
              />
              {plannerEmailError && (
                <p className="text-[12px] mb-3 -mt-2" style={{ color: colors.error }}>{plannerEmailError}</p>
              )}
          ...
          >
            ...
            <div className="flex-1 overflow-auto px-4 py-4">
                ...
                {isEmpty ? (
                  ...
                  >
                    ...
                    <span className="text-xs" style={{ color: plannerColors.successText }}>
                      {totalBudget >= maxBudget ? "Done! Your plan is ready to submit." : "Plan looks good — continue to use your budget."}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                {visibleData.map((month, i) => {
                  const youAmount = month.you === 'basis' ? myCalc.basis : month.you === 'plus' ? myCalc.plus : month.you === 'bonus' ? myCalc.bonus : 0;
                  const partnerAmount = isCouple ? (month.partner === 'basis' ? partnerCalc.basis : month.partner === 'plus' ? partnerCalc.plus : month.partner === 'bonus' ? partnerCalc.bonus : 0) : 0;
                  const rowSum = youAmount + partnerAmount;
                  
                  return (
                    <div key={i} className="flex items-center" style={{ gap: 8, height: 44 }}>
                      <div style={{ width: 24 }}>
                        <span className="text-xs font-medium" style={{ color: colors.text }}>{i + 1}</span>
                      </div>
                      ...
                      >
                        {month.you === 'none' ? (
                          ...
                          <>
                            <svg className="w-3 h-3" style={{ color: colors.textDark }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                      ...
                      <div style={{ width: 72, marginLeft: 12 }} className="flex items-center gap-1">
                        {rowSum > 0 ? (
                          <>
                            <svg className="w-3 h-3 shrink-0" style={{ color: colors.text }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      ...
      >
        {[
          ...
          <div key={i} className="flex items-start gap-2.5">
            <svg className="w-4 h-4 mt-0.5 shrink-0" style={{ color: '#C0630B' }} fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
            <span className="text-[15px] leading-snug" style={{ color: colors.text }}>
              {item.text} <span className="font-medium" style={{ color: colors.textDark }}>{item.bold}</span>
  ...
  const CtaCard = () => {
    ...
    const WAITLIST_STATES = [
      'Baden-Württemberg', 'Bayern', 'Hessen', 'Mecklenburg-Vorpommern', 'Nordrhein-Westfalen',
    ];
        ...
        <div className="py-4">
          ...
          >
            <div className="p-6">
              ...
              <div 
                ...
                style={{ backgroundColor: colors.tile }}
              >
                <svg className="w-6 h-6" style={{ color: colors.textDark }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
          ...
          >
            <div className="flex items-center shrink-0">
              {[1,2,3,4,5].map(i => (
                <span key={i} className="text-[12px]" style={{ color: colors.stars }}>★</span>
              ))}
            </div>
          ...
          >
            <div className="p-6">
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                >
                  <svg className="w-3.5 h-3.5" style={{ color: colors.success }} fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-[12px] font-medium" style={{ color: colors.success }}>{selectedState}</span>
                </div>
              </div>
              ...
              <div className="space-y-2 mb-5">
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 shrink-0" style={{ color: colors.success }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-[14px]" style={{ color: colors.textDark }}>Official Elterngeld form (pre-filled)</span>
                </div>
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 shrink-0" style={{ color: colors.success }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-[14px]" style={{ color: colors.textDark }}>Personalized document checklist</span>
                </div>
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 shrink-0" style={{ color: colors.success }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-[14px]" style={{ color: colors.textDark }}>Instant PDF download</span>
                </div>
              </div>
              
              {/* Price */}
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-[24px] font-bold" style={{ color: colors.textDark }}>€39</span>
                <span className="line-through text-[14px]" style={{ color: colors.text }}>€49</span>
                <span className="text-[13px]" style={{ color: colors.text }}>one-time</span>
              </div>
              ...
              {waitlistSubmitted ? (
                <div className="flex items-center gap-2 py-3">
                  <div 
                    ...
                    style={{ backgroundColor: 'rgba(26, 182, 137, 0.15)' }}
                  >
                    <svg className="w-4 h-4" style={{ color: colors.success }} fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                ...
                <>
                  ...
                  />
                  {waitlistError && (
                    <p className="text-[12px] mb-3 -mt-2" style={{ color: colors.error }}>{waitlistError}</p>
                  )}
  ...
  const SummaryBox = () => {
    ...
    const partnerTotalMoney = (partnerBasis * partnerCalc.basis) + (partnerPlus * partnerCalc.plus) + (partnerBonus * partnerCalc.bonus);
    ...
    const renderModelPills = (basis: number, plus: number, bonus: number) => (
      <div className="flex gap-1 flex-wrap">
        {basis > 0 && (
          <span className="px-2 py-0.5 rounded-full text-[11px] font-medium" style={{ backgroundColor: 'rgba(192, 99, 11, 0.25)', color: colors.textDark }}>{basis} Basis</span>
        )}
        {plus > 0 && (
          <span className="px-2 py-0.5 rounded-full text-[11px] font-medium" style={{ backgroundColor: 'rgba(252, 99, 27, 0.25)', color: colors.textDark }}>{plus} Plus</span>
        )}
        {bonus > 0 && (
          <span className="px-2 py-0.5 rounded-full text-[11px] font-medium" style={{ backgroundColor: 'rgba(255, 189, 240, 0.35)', color: colors.textDark }}>{bonus} Bonus</span>
        )}
        {basis === 0 && plus === 0 && bonus === 0 && (
          <span className="text-[12px]" style={{ color: colors.text }}>—</span>
        )}
      </div>
    );
      ...
      <div className="py-2">
        ...
        >
          ...
          <div className="px-4 py-2" style={{ backgroundColor: colors.buttonDark }}>
            <span className="text-[12px] font-semibold" style={{ color: colors.white }}>Your Elterngeld Plan</span>
          </div>
          ...
          <div className="px-4 py-3">
            ...
            <div className="pb-3" style={{ borderBottom: `1px solid rgba(87, 83, 78, 0.3)` }}>
              <div className="flex justify-between items-center mb-2">
                <span className="text-[12px] font-medium" style={{ color: colors.textDark }}>You</span>
                <span className="text-[12px]" style={{ color: colors.text }}>Income: €{sliderValue.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center">
                {renderModelPills(youBasis, youPlus, youBonus)}
                <span className="text-[14px] font-semibold" style={{ color: colors.textDark }}>€{youTotalMoney.toLocaleString()}</span>
              </div>
            </div>
            ...
            {isCouple && (
              <div className="py-3" style={{ borderBottom: `1px solid rgba(87, 83, 78, 0.3)` }}>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[12px] font-medium" style={{ color: colors.textDark }}>Partner</span>
                  <span className="text-[12px]" style={{ color: colors.text }}>Income: €{partnerSliderValue.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
                  {renderModelPills(partnerBasis, partnerPlus, partnerBonus)}
                  <span className="text-[14px] font-semibold" style={{ color: colors.textDark }}>€{partnerTotalMoney.toLocaleString()}</span>
                </div>
              </div>
            )}
            ...
            <div className="pt-3 flex justify-between items-center">
              <div>
                <span className="text-[12px] font-semibold" style={{ color: colors.textDark }}>Total</span>
                <span className="text-[12px] ml-2" style={{ color: colors.text }}>{totalMonths} months</span>
              </div>
              <span className="text-[16px] font-bold" style={{ color: colors.textDark }}>€{totalMoney.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  const SummaryCard = () => {
    ...
    const totalMoney = (youBasis * myCalc.basis) + (youPlus * myCalc.plus) + 
                       (youBonus * myCalc.bonus) + (partnerBasis * partnerCalc.basis) + 
                       (partnerPlus * partnerCalc.plus) + (partnerBonus * partnerCalc.bonus);

    // Build one-line breakdown: "14 Monate Basis á €1.200, 4 Monate Plus á €600"
    const formatOneLine = (basis: number, plus: number, bonus: number, calc: { basis: number, plus: number, bonus: number }) => {
      const parts = [];
      ...
      <div className="py-4">
        ...
        <div className="rounded-xl p-4" style={{ backgroundColor: colors.tile }}>
          ...
          <div className="space-y-2 text-[14px]">
            <div className="flex justify-between items-start py-2" style={{ borderBottom: `1px solid ${colors.border}` }}>
              <span className="font-semibold shrink-0 mr-4" style={{ color: colors.textDark }}>You</span>
              <span className="text-right" style={{ color: colors.text }}>{formatOneLine(youBasis, youPlus, youBonus, myCalc)}</span>
            </div>
            {data.applicationType === 'couple' && (
              <div className="flex justify-between items-start py-2" style={{ borderBottom: `1px solid ${colors.border}` }}>
                <span className="font-semibold shrink-0 mr-4" style={{ color: colors.textDark }}>Partner</span>
                <span className="text-right" style={{ color: colors.text }}>{formatOneLine(partnerBasis, partnerPlus, partnerBonus, partnerCalc)}</span>
              </div>
            )}
          </div>
          ...
          <div className="mt-3 pt-3" style={{ borderTop: `2px solid ${colors.text}` }}>
            <div className="flex justify-between items-baseline">
              <span className="text-[14px]" style={{ color: colors.text }}>Estimated total</span>
              <span className="text-xl font-bold" style={{ color: colors.textDark }}>€{totalMoney.toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        <p className="text-[12px] mt-3 px-1" style={{ color: colors.text }}>
          In months 1-2, mothers typically receive maternity pay from their health insurance. This counts as Basis and is deducted – you won't get both amounts.
        </p>
      </div>
    );
  };
  ...
  const InfoTagsComponent = ({ tags, description }: { tags: InfoTag[], description?: string }) => (
    <div className="mt-4">
      {description && (
        <p className="text-[15px] mb-2" style={{ color: colors.text }}>{description}</p>
      )}
  ...
  const buttonIcons: Record<string, React.ReactNode> = {
    'eu': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path d="M4 4v16M4 4h12l-2 4 2 4H4" /></svg>,
    'passport': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><rect x="4" y="2" width="16" height="20" rx="2" /><circle cx="12" cy="10" r="3" /><path d="M8 17h8" /></svg>,
    'baby': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><circle cx="12" cy="8" r="5" /><path d="M4 20c0-4 4-6 8-6s8 2 8 6" /></svg>,
    'twins': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><circle cx="8" cy="8" r="4" /><circle cx="16" cy="8" r="4" /><path d="M2 20c0-3 2.5-5 6-5s6 2 6 5M10 20c0-3 2.5-5 6-5s6 2 6 5" /></svg>,
    'triplets': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><circle cx="12" cy="5" r="3" /><circle cx="6" cy="11" r="3" /><circle cx="18" cy="11" r="3" /><path d="M12 8v4M6 14v4M18 14v4" /></svg>,
    'child': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><circle cx="12" cy="7" r="4" /><path d="M5 21v-2a7 7 0 0114 0v2" /></svg>,
    'children': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><circle cx="9" cy="7" r="3" /><circle cx="17" cy="9" r="2.5" /><path d="M3 21v-2c0-2.5 2-4.5 6-4.5s6 2 6 4.5v2M14 21v-1.5c0-2 1.5-3.5 4.5-3.5" /></svg>,
    'couple': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><circle cx="9" cy="7" r="4" /><circle cx="17" cy="9" r="3" /><path d="M3 21v-2c0-2.5 3-5 6-5 1.5 0 3 .5 4 1.5M17 21v-2c0-1.5 1-3 3-3" /></svg>,
    'single': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><circle cx="12" cy="7" r="4" /><path d="M5 21v-2a7 7 0 0114 0v2" /></svg>,
    'briefcase': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2" /></svg>,
    'home': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path d="M3 12l9-9 9 9M5 10v10a1 1 0 001 1h3v-6h6v6h3a1 1 0 001-1V10" /></svg>,
    'check': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" /></svg>,
    'x': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path d="M6 6l12 12M6 18L18 6" /></svg>,
    'heart': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z" /></svg>,
    'calendar': <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /></svg>,
  };
  const ButtonIcon = React.memo(({ name }: { name: string }) => buttonIcons[name] || null);

  const ButtonOptions = React.memo(({ options, onSelect }: { options: ButtonOption[], onSelect: (value: string, label: string) => void }) => (
    <div className="space-y-2">
      {options.map((opt, i) => (
        <button
          key={i}
        ...
        >
          <div className="flex items-center gap-3">
            {opt.icon && <span style={{ color: colors.textDark }}><ButtonIcon name={opt.icon} /></span>}
            <div>
              <span className="text-[14px] font-medium" style={{ color: colors.textDark }}>{opt.label}</span>
              {opt.sub && <p className="text-[12px] mt-0.5" style={{ color: colors.text }}>{opt.sub}</p>}
            </div>
          </div>
          {opt.note && (
            <span 
              ...
              style={{ 
                backgroundColor: opt.accent === 'basis' ? 'rgba(192, 99, 11, 0.3)' : opt.accent === 'bonus' ? colors.bonus : colors.tile,
  ...
  const renderMessage = (msg: FlowMessage, i: number) => {
      ...
      return (
        ...
        >
          <div className="rounded-2xl rounded-br-sm px-4 py-2.5 max-w-[80%]" style={{ backgroundColor: colors.userBubble }}>
            <span className="text-[15px]" style={{ color: colors.textDark }}>{msg.content}</span>
          </div>
        </div>
      );
    }
    ...
    if (msg.type === 'bot') {
      ...
      return (
        <div key={i} className={`${isQuestion ? "py-1.5 mt-4" : "py-1"} ${hasOnlyExpander ? "mb-4" : ""}`}>
          ...
          {msg.subtext && !isCurrentlyStreaming && (
    ...
    if (msg.type === 'component') {
      switch(msg.component) {
        case 'introChecklist': return <IntroChecklist key={i} />;
        case 'calculation': return <CalculationCard key={i} />;
        case 'planner': return <React.Fragment key={i}>{PlannerComponent()}</React.Fragment>;
        case 'checkmarks': return <CheckmarksComponent key={i} />;
        case 'summaryBox': return <SummaryBox key={i} />;
        case 'ctaCard': return <CtaCard key={i} />;
  ...
  const renderInput = () => {
    ...
    if (showInput.input === 'date') return (
      <DateInputComponent
        value={dateValue}
    ...
    if ((showInput.input as string) === 'visa') return (
      <VisaSelectorComponent
        onSelect={(visaId, label) => handleInput(visaId, label)}
      />
    );
  ...
  if (showPdfFlow) {
    return (
      <div className="h-screen flex flex-col overflow-hidden" style={{ backgroundColor: colors.background }}>
          ...
          <div className="px-5 py-3">
            <div className="max-w-lg mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-sm font-semibold" style={{ color: colors.textDark }}>Your Elterngeld Application</h1>
              </div>
              <span className="text-xs" style={{ color: colors.text }}>Step 1/8</span>
            </div>
          </div>
        </div>
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="px-5 py-6">
            <div className="max-w-lg mx-auto">
              ...
              <div className="rounded-xl overflow-hidden mb-3" style={{ backgroundColor: colors.white, border: `1.5px solid #F2F53A` }}>
                <div className="px-4 py-3.5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium" style={{ color: colors.textDark }}>Child information</span>
                  </div>
                  <svg className="w-4 h-4" style={{ color: colors.text, transform: 'rotate(180deg)' }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
                <div className="px-4 pb-4 pt-3 border-t space-y-4" style={{ borderColor: colors.border }}>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: colors.text }}>First name(s)</label>
                      <input type="text" className="w-full px-3 py-2.5 rounded-lg text-sm" style={{ backgroundColor: colors.white, border: `1.5px solid ${colors.border}`, color: colors.textDark }} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: colors.text }}>Last name</label>
                      <input type="text" className="w-full px-3 py-2.5 rounded-lg text-sm" style={{ backgroundColor: colors.white, border: `1.5px solid ${colors.border}`, color: colors.textDark }} />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-medium mb-1.5" style={{ color: colors.text }}>Date of birth / Due date</label>
                    <input type="date" className="w-full px-3 py-2.5 rounded-lg text-sm" style={{ backgroundColor: colors.white, border: `1.5px solid ${colors.border}`, color: colors.textDark }} />
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl overflow-hidden mb-3" style={{ backgroundColor: colors.white }}>
                <div className="px-4 py-3.5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium" style={{ color: colors.textDark }}>Special circumstances</span>
                  </div>
                  <svg className="w-4 h-4" style={{ color: colors.text }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
              </div>
              ...
              <div className="rounded-xl overflow-hidden mb-3" style={{ backgroundColor: colors.white }}>
                <div className="px-4 py-3.5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium" style={{ color: colors.textDark }}>Relationship to child</span>
                  </div>
                  <svg className="w-4 h-4" style={{ color: colors.text }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
        ...
        {/* Footer */}
        <footer className="flex-shrink-0 border-t" style={{ backgroundColor: colors.background, borderColor: colors.border }}>
  ...
  return (
    <>
      ...
      {!showChat && (
      <div className="flex-shrink-0 px-5 pb-2 pt-3 relative z-10" style={{ backgroundColor: colors.background, borderTop: `1px solid ${colors.border}` }}>
                  ...
                  <div className="space-y-0">
                    {[
                      ...
                      <button
                        key={i}
                        onClick={() => {
                          const demoResponse = 'Thanks for your question! This is a demo response. In the real implementation, this would connect to an AI backend to answer your **Elterngeld** questions with detailed information about eligibility, calculations, and application process.';
                      ...
                      >
                        ...
                        >
                          {item.icon === 'help' && (
                            <svg className="w-[18px] h-[18px]" fill="none" stroke={colors.text} strokeWidth={1.5} viewBox="0 0 24 24">
            ...
            <div className="max-w-lg mx-auto">
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  >
                    <svg className="w-4 h-4" fill="none" stroke={chatInput.trim() && !isChatStreaming ? colors.white : colors.text} strokeWidth={2} viewBox="0 0 24 24">
