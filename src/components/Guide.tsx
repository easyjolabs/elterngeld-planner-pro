...
interface FlowMessage {
  ...
  component?: "calculation" | "planner" | "checkmarks" | "summary" | "ctaCard" | "introChecklist" | "continue" | "ineligible" | "summaryBox";
  pause?: boolean;
  pauseLabel?: string;
  key?: string;
  isQuestion?: boolean;
...
const visaTypes: VisaType[] = [{
  id: "uk_pre_brexit",
  label: "British – arrived before Jan 2021",
  category: "special",
  status: "eligible"
}, {
  id: "turkey_insured",
  ...
  id: "ict_card",
  label: "ICT-Karte (Intra-Company Transfer)",
  category: "work",
  status: "eligible"
}, {
  id: "work_permit",
  ...
  id: "training",
  label: "Training Visa (Ausbildung)",
  category: "study",
  status: "not_eligible",
  paragraph: "§16e"
}, {
  id: "ukraine",
  label: "Ukraine Residence Permit",
...
// Flow starts with citizenshipResponse (handled dynamically based on Start Screen answer)
{
  ...
  options: [{
    value: "under",
    label: "No, under €175,000",
    icon: "check"
  }, {
    value: "over",
    label: "Yes, over €175,000",
    icon: "x"
  }]
}, {
  type: "user"
  ...
  options: [{
    value: "couple",
    label: "Applying as a couple",
    icon: "couple"
  }, {
    value: "single",
    label: "Applying as a single parent",
    icon: "single",
    note: "All 14 months"
  }]
}, {
  ...
  component: "planner",
  pause: true,
  pauseLabel: "Continue to application →"
}, {
  type: "bot",
...
// HELPER FUNCTIONS
// ===========================================
const LightbulbIcon = () => <svg className="w-4 h-4 inline-block mr-1 -mt-0.5" style={{
  color: colors.text
...
const formatText = (text: string | undefined, onOpenChat?: (prefill?: string) => void): React.ReactNode => {
  ...
  const processBulletGroup = () => {
    ...
    result.push(<ul key={`ul-${result.length}`} className="space-y-0.5" style={{
      paddingLeft: "1rem"
    }}>
        {bulletGroup.map((item, idx) => {
        const content = item.replace(/^[•]\s*/, "");
        return <li key={idx} className="relative" style={{
          listStyle: "none"
        }}>
  ...
  const processLine = (line: string, lineIndex: number): React.ReactNode => {
    ...
    const withLinks = linkParts.map((part, i) => {
      if (i % 2 === 1) {
        ...
        return <button key={`link-${i}`} onClick={() => onOpenChat?.(prefillQuestion || undefined)} className="font-semibold underline hover:opacity-70 transition-opacity" style={{
      ...
      return boldParts.map((boldPart, j) => j % 2 === 1 ? <strong key={`${i}-${j}`} className="font-semibold" style={{
        color: colors.textDark
      }}>
            {boldPart}
          </strong> : boldPart);
  ...
  lines.forEach((line, lineIndex) => {
    ...
    if (isBullet) {
      ...
      processBulletGroup();
      if (line.trim()) {
        result.push(<span key={lineIndex}>
...
const SliderInputComponent: React.FC<SliderInputProps & {
  ...
  return <div className="p-5" style={{
    backgroundColor: colors.white,
    ...
    border: `1px solid ${colors.border}`
      ...
      <div ref={trackRef} className="relative h-10 cursor-pointer select-none touch-none" role="slider" aria-valuemin={SLIDER.MIN} aria-valuemax={SLIDER.MAX} aria-valuenow={value} aria-label="Income slider" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}>
        <div className="absolute top-1/2 left-0 right-0 flex justify-between px-0" style={{
        transform: "translateY(-50%)"
      }}>
          {tickValues.map((tick, i) => <div key={i} className="w-0.5 h-1.5 rounded-full" style={{
          backgroundColor: value >= tick ? colors.basis : colors.border,
          opacity: value >= tick ? 0.5 : 0.3
        }} />)}
        </div>

        <div className="absolute top-1/2 left-0 right-0 h-0.5 rounded-full -translate-y-1/2" style={{
        backgroundColor: colors.textDark
      }} />

        <div className="absolute top-1/2 left-0 h-0.5 rounded-full -translate-y-1/2 pointer-events-none" style={{
        backgroundColor: colors.textDark,
        width: `${percent}%`
      }} />

        <div className="absolute top-1/2 w-5 h-5 rounded-full pointer-events-none -translate-y-1/2" style={{
      ...
      <button onClick={() => onConfirm(value, `€${value.toLocaleString()}`)} className="w-full mt-6 flex items-center justify-between" style={{
...
const IncomeStepper: React.FC<StepperProps> = ({
  value,
  label,
  onChange
}) => {
  ...
  return <div className="flex items-center justify-between flex-1">
      ...
      <div className="flex items-center gap-1">
        <button onClick={decrease} className="w-5 h-5 flex items-center justify-center" style={{
        color: colors.textDark,
        cursor: "pointer",
        fontSize: "16px"
      }}>
        ...
        <button onClick={increase} className="w-5 h-5 flex items-center justify-center" style={{
        color: colors.textDark,
        cursor: "pointer",
        fontSize: "16px"
      }}>
...
const typeStyles = {
  ...
  basis: {
    bg: "rgba(192, 99, 11, 0.25)"
  },
  plus: {
    bg: "rgba(252, 99, 27, 0.25)"
  },
  bonus: {
    bg: "rgba(255, 228, 76, 0.35)"
  }
} as const;
const TypeSelectComponent: React.FC<TypeSelectProps> = ({
  value,
  onChange,
  hasError,
  isSingleParent
}) => <div className="relative w-full">
    <select value={value} onChange={e => onChange(e.target.value)} className="h-6 pl-2.5 pr-5 rounded-full font-medium appearance-none cursor-pointer w-full" aria-label="Select benefit type" style={{
    fontSize: "11px",
    ...
    <svg className="absolute right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 pointer-events-none" style={{
    color: colors.textDark
  }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
...
const buttonIcons: Record<string, React.ReactNode> = {
  eu: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <path d="M4 4v16M4 4h12l-2 4 2 4H4" />
    </svg>,
  passport: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <rect x="4" y="2" width="16" height="20" rx="2" />
      <circle cx="12" cy="10" r="3" />
      <path d="M8 17h8" />
    </svg>,
  baby: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <circle cx="12" cy="8" r="5" />
      <path d="M4 20c0-4 4-6 8-6s8 2 8 6" />
    </svg>,
  twins: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <circle cx="8" cy="8" r="4" />
      <circle cx="16" cy="8" r="4" />
      <path d="M2 20c0-3 2.5-5 6-5s6 2 6 5M10 20c0-3 2.5-5 6-5s6 2 6 5" />
    </svg>,
  triplets: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <circle cx="12" cy="5" r="3" />
      <circle cx="6" cy="11" r="3" />
      <circle cx="18" cy="11" r="3" />
      <path d="M12 8v4M6 14v4M18 14v4" />
    </svg>,
  child: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <circle cx="12" cy="7" r="4" />
      <path d="M5 21v-2a7 7 0 0114 0v2" />
    </svg>,
  children: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <circle cx="9" cy="7" r="3" />
      <circle cx="17" cy="9" r="2.5" />
      <path d="M3 21v-2c0-2.5 2-4.5 6-4.5s6 2 6 4.5v2M14 21v-1.5c0-2 1.5-3.5 4.5-3.5" />
    </svg>,
  couple: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <circle cx="9" cy="7" r="4" />
      <circle cx="17" cy="9" r="3" />
      <path d="M3 21v-2c0-2.5 3-5 6-5 1.5 0 3 .5 4 1.5M17 21v-2c0-1.5 1-3 3-3" />
    </svg>,
  single: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <circle cx="12" cy="7" r="4" />
      <path d="M5 21v-2a7 7 0 0114 0v2" />
    </svg>,
  briefcase: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <rect x="2" y="7" width="20" height="14" rx="2" />
      <path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2" />
    </svg>,
  home: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <path d="M3 12l9-9 9 9M5 10v10a1 1 0 001 1h3v-6h6v6h3a1 1 0 001-1V10" />
    </svg>,
  check: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <path d="M5 13l4 4L19 7" />
    </svg>,
  x: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <path d="M6 6l12 12M6 18L18 6" />
    </svg>,
  heart: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z" />
    </svg>,
  calendar: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
...
const ButtonOptions = React.memo(({
  ...
  return <div className="space-y-2">
      ...
      return <button key={i} onClick={() => onSelect(opt.value, opt.label)} className={`w-full transition-all flex items-center hover:border-stone-400 ${shouldCenter ? "justify-between" : "justify-between text-left"}`} style={{
        backgroundColor: colors.white,
            ...
            color: colors.textDark
              ...
              color: colors.textDark
                ...
                <div className="flex items-center gap-2">
                  {opt.note && <span className="px-2.5 py-1 rounded-full" style={{
              fontSize: fontSize.tiny,
              fontWeight: 500,
...
// ===========================================
const VisaSelectorComponent: React.FC<VisaSelectorProps> = ({
  onSelect
}) => {
  ...
  const categoryOptions = [{
    id: "work",
    label: "Work permit",
    sub: "Blue Card, Niederlassungserlaubnis..."
  }, {
    id: "study",
    label: "Student / Job seeker",
    sub: "Student visa, Job seeker..."
  }, {
    id: "humanitarian",
    label: "Humanitarian",
    sub: "Ukraine, Asylum..."
    ...
    sub: "UK pre-Brexit, Turkish/Moroccan..."
  }, {
    id: "other",
  ...
  if (!selectedCategory) {
    return <div className="py-4 space-y-2">
        {categoryOptions.map(cat => <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className="w-full p-3.5 text-left transition-all hover:border-stone-400" style={{
        backgroundColor: colors.white,
        border: `1.5px solid ${colors.border}`,
        borderRadius: ui.buttonRadius
      }}>
  ...
  return <div className="py-4">
      <button onClick={() => setSelectedCategory(null)} className="flex items-center gap-1.5 mb-4 hover:opacity-70" style={{
      color: colors.text,
      fontSize: fontSize.button
    }}>
      ...
      <div className="space-y-2">
        {categoryVisas.map(visa => <button key={visa.id} onClick={() => handleSelectVisa(visa.id)} className="w-full p-3.5 text-left transition-all hover:border-stone-400" style={{
        backgroundColor: colors.white,
        border: `1.5px solid ${colors.border}`,
        borderRadius: ui.buttonRadius
      }}>
  ...
  return <div style={{
    ...
    borderBottom: isLast ? `1px solid ${colors.border}` : "none"
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="w-full py-2 flex items-center justify-between text-left">
          ...
          <svg className="w-4 h-4 shrink-0 transition-transform duration-200" style={{
        color: colors.text,
        ...
        <div className="overflow-hidden transition-all duration-200 ease-out" style={{
      maxHeight: isOpen ? "500px" : "0px",
      opacity: isOpen ? 1 : 0
    }}>
          <div className="pb-2 pl-5 leading-relaxed" style={{
        fontSize: fontSize.button,
        color: colors.text
      }}>
...
const ElterngeldGuide: React.FC<ElterngeldGuideProps> = ({
  onOpenChat
}) => {
  const {
    user,
  ...
  const [stepHistory, setStepHistory] = useState<Array<{
    ...
    savedShowInput: FlowMessage | null;
  }>>([]);
  ...
  const [plannerData, setPlannerData] = useState<PlannerMonth[]>(Array.from({
    length: 32
  }, () => ({
    you: "none",
    partner: "none"
  ...
  }, [step, messages, stepHistory, showInput, isPaused, data, sliderValue, partnerSliderValue, plannerData, plannerMonths, selectedState, workPartTime, partTimeIncome, partnerPartTimeIncome, lastUserMessageIndex, ctaStep]);
  ...
  const jumpToPlanner = () => {
    setMessages([{
      type: "bot",
      content: "Now you can plan how to split your Elterngeld months."
    }, {
      type: "component",
      component: "planner"
    }]);
  ...
  const jumpToCta = () => {
    ...
    setMessages([{
      type: "bot",
      content: "The hardest part is done – you've figured out your Elterngeld strategy! Now just add your personal details and we'll generate your official application."
    }, {
      type: "component",
      component: "ctaCard"
    }]);
...
If your partner can't claim, you may qualify as a **single parent** and use all 14 months yourself.`
      ...
      case "calculationIntro":
        response = {
          type: "bot",
          content: "Here's your estimated Elterngeld based on your income:"
        };
        break;
      case "plannerIntro":
        const plannerText = data.applicationType === "couple" ? "Now let's plan when each of you takes your months." : "Now let's plan your months.";
        const plannerMsg = {
          type: "bot" as const,
          content: plannerText,
          pause: true,
          pauseLabel: "Start planning →"
        };
        streamMessage(plannerMsg, () => {
          setShowInput({
            type: "component",
            component: "continue",
            pauseLabel: plannerMsg.pauseLabel
          });
          setIsPaused(true);
        });
        return;
          ...
          streamMessage({
            ...
            setShowInput({
              ...
              options: [{
                ...
                sub: "For planning purposes",
                icon: "check"
              }, {
                value: "find_stelle",
                label: "Find my Elterngeldstelle",
                sub: "Get individual advice",
                icon: "home"
              }]
            });
          });
    ...
    switch (key) {
        ...
        if (!visa) {
          ...
          if (visa.id === "other") {
            ...
            streamMessage({
              ...
              isQuestion: true,
              subtext: "With your visa type, you're only eligible if you're working, in Elternzeit, or receiving unemployment benefits (ALG)."
            }, () => {
              setShowInput({
                type: "bot",
                input: "buttons",
                ...
                options: [{
                  value: "employed",
                  label: "Yes, I'm employed",
                  icon: "briefcase"
                }, {
                  value: "elternzeit",
                  label: "I'm in Elternzeit",
                  icon: "baby"
                }, {
                  value: "alg",
                  label: "I receive ALG (unemployment)",
                  icon: "home"
                }, {
                  value: "no",
                  label: "No, none of these",
                  icon: "x"
                }]
              });
            });
            ...
            streamMessage({
              ...
              setShowInput({
                ...
                options: [{
                  ...
                  value: "15months",
                  label: "I've been in Germany 15+ months",
                  icon: "home"
                }, {
                  value: "neither",
                  label: "Neither",
      ...
      case "conditionalVisaResponse":
        if (["student", "qualification", "job_seeker"].includes(currentData.visaType || "")) {
          if (currentData.isEmployed === "no") {
            ...
            response = {
              type: "bot",
              content: "Since you're employed, you **likely qualify** for Elterngeld!"
            };
          } else if (currentData.isEmployed === "elternzeit") {
            response = {
              type: "bot",
              content: "Since you're in Elternzeit, you **likely qualify** for Elterngeld!"
            };
          } else {
            response = {
              type: "bot",
              content: "Since you're receiving ALG, you **likely qualify** for Elterngeld!"
            };
          }
      ...
      case "ineligibleChoiceResponse":
        if (currentData.ineligibleChoice === "continue" || currentData.continueAfterStelle === "yes") {
          ...
          streamMessage({
            type: "bot",
            content: "I've opened the **Elterngeldstelle finder** for you. They can give you personalized advice on your specific situation."
          }, () => {
            setTimeout(() => {
              streamMessage({
                type: "bot",
                content: "Want to continue with the planning tool anyway?",
                isQuestion: true
              }, () => {
                setShowInput({
                  type: "bot",
                  ...
                  options: [{
                    value: "yes",
                    label: "Yes, continue planning",
                    icon: "check"
                  }, {
                    value: "no",
                    label: "No, I'll stop here",
                    icon: "x"
                  }]
                });
              });
          ...
          streamMessage({
            type: "bot",
            content: "No problem! Feel free to come back anytime. Good luck with your application!"
          }, () => {
            setShowInput({
              type: "end"
            } as any);
  ...
  const categoryIcons: Record<string, React.ReactNode> = {
    Residence: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const CategoryHeader = React.memo(({
    ...
    label: string;
      ...
      <span className="font-bold uppercase flex items-center gap-2" style={{
      color: colors.text,
  ...
  const calculationCardContent = React.useMemo(() => {
    ...
    const renderAmount = (current: number, original: number, size: string = fontSize.small, bold: boolean = false) => {
      ...
      return <span style={{
        ...
        fontSize: size,
        fontWeight
      }}>€{current.toLocaleString("de-DE")}</span>;
    };
    const cards = [{
      label: "BASIS",
      you: myCalc.basis,
    ...
    return <div className="py-3">
            ...
            return (
              ...
              >
                ...
                padding: "6px 0"
                  ...
                  </div> : <div className="flex items-center justify-between" style={{
              padding: "6px 0"
            }}>
                    <span style={{
                color: "rgba(0,0,0,0.7)",
        ...
        marginBottom: "12px"
          ...
          <button onClick={() => setWorkPartTime(!workPartTime)} className="w-full flex items-center justify-between cursor-pointer" style={{
          padding: "14px 16px",
          background: "none",
          border: "none"
        }}>
            ...
            gap: "10px"
              ...
              fontSize: "15px",
              fontWeight: 600
            }}>Planning to work part-time?</span>
            </div>
            <div className="relative rounded-full transition-colors duration-200" style={{
            width: "44px",
            height: "24px",
            backgroundColor: workPartTime ? colors.basis : "#E7E5E4"
          }}>
              <span className="absolute rounded-full bg-white transition-transform duration-200" style={{
              width: "20px",
              height: "20px",
              top: "2px",
              left: "2px",
          ...
          padding: "0 16px 16px 16px"
            ...
            marginBottom: "12px"
              ...
              <div className="flex items-center gap-6">
                ...
                {isCouple && <IncomeStepper value={partnerPartTimeIncome} label="Partner" onChange={setPartnerPartTimeIncome} />}
              </div>
            </div>}
        </div>
        ...
        gap: "10px"
          ...
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#000" strokeWidth={1.5} className="shrink-0">
          ...
          lineHeight: 1.4
            ...
            <strong style={{
            fontWeight: 600
          }}>Partnership Bonus</strong>: +€{bonusPerMonth.toLocaleString("de-DE")}
  ...
  // Intro Checklist
  const introChecklistContent = React.useMemo(() => <div className="pt-2 pb-6">
      ...
      border: `1px solid ${colors.border}`
          ...
          {[{
        text: "Your",
        bold: "monthly Elterngeld amount",
        after: "based on your income"
      }, {
        text: "The",
        bold: "smartest way to split months",
        after: "with your partner"
      }, {
        text: "What you need to",
        bold: "complete your application"
      }].map((item, i) => <div key={i} className="flex items-start gap-2.5">
              <svg className="w-4 h-4 mt-0.5 shrink-0" style={{
          color: colors.orange
  ...
  const checkmarksContent = React.useMemo(() => <div className="py-2">
        <div className="flex flex-wrap gap-x-4 gap-y-1">
          {["Eligibility", "Calculation", "Planning"].map((item, i) => <div key={i} className="flex items-center gap-1.5">
  ...
  // CTA Card
  const CtaCard = () => {
    const SUPPORTED_STATES = ["Berlin", "Brandenburg", "Bremen", "Hamburg", "Niedersachsen", "Rheinland-Pfalz", "Saarland", "Sachsen", "Sachsen-Anhalt", "Schleswig-Holstein", "Thüringen"];
    ...
    const reviews = [{
      text: "Saved me hours of confusion!",
      name: "Sarah M.",
      context: "US Expat in Berlin"
    }, {
      text: "Finally understood my Elterngeld options",
      name: "Marco T.",
      context: "Italian in Munich"
    }, {
      text: "The PDF was perfectly filled out",
      name: "Anna K.",
      context: "British in Hamburg"
    }, {
      text: "Worth every cent!",
      name: "David L.",
      context: "American in Frankfurt"
    }];
    ...
    const handleWaitlistSubmit = async () => {
      ...
      const {
        error
      } = await signInWithEmail(waitlistEmail, waitlistConsent);
      if (error) {
    ...
    if (ctaStep === 1) {
      return <div className="py-4">
          ...
          borderRadius: ui.cardRadius
            ...
            <div className="p-6">
              ...
              borderRadius: 12
                ...
                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="mb-4">
                <label className="font-medium mb-1.5 block" style={{
                fontSize: fontSize.small,
                color: colors.textDark
              }}>
                  Select your state
                ...
                position: "relative"
                  ...
                  <select value={selectedState} onChange={e => setSelectedState(e.target.value)} onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()} className="w-full outline-none cursor-pointer" style={{
                  backgroundColor: colors.white,
                  ...
                  MozAppearance: "none"
                    ...
                    {ALL_STATES.map(state => <option key={state} value={state}>
                        {state}
                      </option>)}
                  </select>
                  <svg style={{
                  ...
                  transform: "translateY(-50%)",
                  pointerEvents: "none"
                }} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#78716c" strokeWidth={2}>
                    <path d="M6 9l6 6 6-6" />
                  </svg>
                </div>
              </div>
              ...
              handleContinue();
            }} className="w-full flex items-center justify-between" style={{
              backgroundColor: "#000",
              color: "#fff",
              height: ui.buttonHeight,
              borderRadius: ui.buttonRadius,
          ...
          <div className="mt-4 flex items-center justify-center gap-1.5 transition-opacity duration-300" style={{
          opacity: reviewFade ? 1 : 0
        }}>
            <div className="flex items-center shrink-0">
              {[1, 2, 3, 4, 5].map(i => <span key={i} style={{
              fontSize: fontSize.tiny,
              color: colors.stars
            }}>
                  ★
            ...
            fontSize: fontSize.small,
            color: colors.text
          }}>"{reviews[currentReview].text}"</p>
          </div>

          <button onClick={() => onOpenChat?.()} className="w-full mt-2" style={{
          fontSize: fontSize.small,
          color: colors.text
        }}>
            Have a question? <span style={{
            ...
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                backgroundColor: "rgba(255,255,255,0.5)"
                  ...
                  <svg className="w-3.5 h-3.5" style={{
              ...
              <div className="space-y-2 mb-5">
                {["Official Elterngeld form (pre-filled)", "Personalized document checklist", "Instant PDF download"].map((feature, i) => <div key={i} className="flex items-center gap-2">
                    <svg className="w-4 h-4 shrink-0" style={{
                  color: colors.textDark
              ...
              <button onClick={() => setCtaStep(1)} className="w-full mt-3 flex items-center justify-center gap-1" style={{
              fontSize: "11px",
              color: "rgba(0,0,0,0.6)"
            }}>
          ...
          borderRadius: ui.cardRadius
            ...
            <div className="p-6">
              ...
              borderRadius: 12
                ...
                <svg className="w-6 h-6" fill="none" stroke="#fff" strokeWidth={1.5} viewBox="0 0 24 24">
              ...
              {waitlistSubmitted ? <div className="flex items-center gap-2 py-3">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center" style={{
                backgroundColor: "rgba(26, 182, 137, 0.15)"
              }}>
                ...
                color: colors.textDark
                  ...
                  <input type="email" value={waitlistEmail} onChange={e => {
                setWaitlistEmail(e.target.value);
                setWaitlistError("");
              }} placeholder="your@email.com" className="w-full px-4 py-3 outline-none mb-3" style={{
                backgroundColor: colors.white,
                color: colors.textDark,
                ...
                fontSize: fontSize.subtext
                  ...
                  {waitlistError && <p className="mb-3 -mt-2" style={{
                fontSize: fontSize.tiny,
                color: colors.error
              }}>
                      {waitlistError}
                  ...
                  <label className="flex items-start gap-2.5 mb-4 cursor-pointer">
                    <input type="checkbox" checked={waitlistConsent} onChange={e => setWaitlistConsent(e.target.checked)} className="mt-0.5 w-4 h-4 rounded" style={{
                  accentColor: colors.buttonDark
                }} />
                  ...
                  <button onClick={handleWaitlistSubmit} disabled={waitlistSubmitting} className="w-full flex items-center justify-center gap-2" style={{
                backgroundColor: colors.buttonDark,
                color: colors.white,
                height: ui.buttonHeight,
                borderRadius: ui.buttonRadius,
  ...
  const SummaryBox = () => {
    ...
    const countMonths = (person: "you" | "partner", type: string) => visibleData.filter(m => m[person] === type).length;
    ...
    const isCouple = data.applicationType === "couple";
    const renderModelPills = (basis: number, plus: number, bonus: number) => <div className="flex gap-1 flex-wrap">
        {basis > 0 && <span className="px-2 py-0.5 rounded-full font-medium" style={{
        backgroundColor: "rgba(192, 99, 11, 0.25)",
        color: colors.textDark,
        fontSize: "11px"
      }}>
            {basis} Basis
          </span>}
        {plus > 0 && <span className="px-2 py-0.5 rounded-full font-medium" style={{
        backgroundColor: "rgba(252, 99, 27, 0.25)",
        color: colors.textDark,
        fontSize: "11px"
      }}>
            {plus} Plus
          </span>}
        {bonus > 0 && <span className="px-2 py-0.5 rounded-full font-medium" style={{
        backgroundColor: "rgba(255, 228, 76, 0.35)",
        color: colors.textDark,
        fontSize: "11px"
      }}>
            {bonus} Bonus
          </span>}
        {basis === 0 && plus === 0 && bonus === 0 && <span style={{
        fontSize: fontSize.tiny,
        color: colors.text
      }}>—</span>}
      </div>;
    return <div className="py-2">
        ...
        borderRadius: ui.cardRadius
          ...
          <div className="px-4 py-3">
            ...
            {isCouple && <div className="py-3" style={{
            borderBottom: `1px solid rgba(87, 83, 78, 0.3)`
          }}>
                <div className="flex justify-between items-center mb-2">
                  <span className="font-medium" style={{
  ...
  const SliderInput = ({
    ...
    return <SliderInputComponent value={value} onChange={setValue} onConfirm={(val, display) => handleInput(val, `${displayPrefix}${display}`)} label={label} />;
  };
  ...
  const renderMessage = (msg: FlowMessage, i: number) => {
    ...
    if (msg.type === "user") {
      ...
      return <div key={i} ref={isLastUserMessage ? lastUserMessageRef : null} className="flex justify-end py-2">
          <div className="rounded-2xl rounded-br-sm px-4 py-2.5 max-w-[80%] bg-[#f1ede4]" style={{
    ...
    if (msg.type === "bot") {
      ...
      return <div key={i} className={`${isQuestion ? "py-1.5 mt-4" : "py-1"} ${hasOnlyExpander ? "mb-4" : ""}`}>
          {hasContent && <p className={isQuestion ? "leading-relaxed font-semibold pl-3" : "leading-relaxed"} style={{
          color: colors.textDark,
          ...
          fontSize: isQuestion ? fontSize.question : fontSize.body
              ...
              {isCurrentlyStreaming && <span className="inline-block w-2 h-2 ml-1 rounded-full align-middle" style={{
            backgroundColor: colors.text,
            animation: "pulse 1.2s ease-in-out infinite"
          }} />}
            </p>}
          {msg.subtext && !isCurrentlyStreaming && (msg.subtext.includes("[info:") ? <>
                {msg.subtext.split("[info:")[0].trim() && <p className={`mt-2 ${isQuestion ? "pl-3" : ""}`} style={{
            fontSize: fontSize.subtext,
            color: colors.text
          }}>
            ...
            return blocks.map((block, idx) => {
              ...
              return <div key={idx} className={`${idx === 0 ? "mt-3" : ""} ${isQuestion ? "pl-3" : ""}`}>
            ...
            color: colors.text
                ...
                <p className={`mt-2 ${isQuestion ? "pl-3" : ""}`} style={{
            fontSize: fontSize.subtext,
            color: colors.text
          }}>
      ...
      switch (msg.component) {
        ...
        case "planner":
          return <React.Fragment key={i}>
              <ElterngeldPlanner plannerData={plannerData} setPlannerData={setPlannerData} plannerMonths={plannerMonths} applicationType={data.applicationType as "couple" | "single"} premature={data.premature} myCalc={myCalc} partnerCalc={partnerCalc} isLoggedIn={!!user} onSaveClick={() => {
              saveSessionToLocalStorage();
              setShowPlannerSaveInput(true);
            }} fullscreen={plannerFullscreen} onFullscreenToggle={() => setPlannerFullscreen(!plannerFullscreen)} />
            </React.Fragment>;
        case "checkmarks":
  ...
  const renderInput = () => {
    ...
    if (showInput.component === "continue") {
      ...
      return <button onClick={() => handleContinue(label)} className="w-full flex items-center justify-between" style={{
        backgroundColor: colors.buttonDark,
        color: colors.white,
        height: ui.buttonHeight,
        borderRadius: ui.buttonRadius,
    ...
    if (showInput.input === "date") return <DateInputComponent value={dateValue} onChange={setDateValue} onConfirm={(isoDate, displayDate) => handleInput(isoDate, displayDate)} />;
    if (showInput.input === "slider") return <SliderInput person={showInput.person || "you"} />;
    if (showInput.input as string === "visa") return <VisaSelectorComponent onSelect={(visaId, label) => handleInput(visaId, label)} />;
    return null;
  };
  ...
  if (showPdfFlow) {
    return <div className="flex flex-col overflow-hidden" style={{
      ...
      height: `calc(100vh - ${HEADER_HEIGHT}px)`
        ...
        borderBottom: `1px solid ${colors.border}`
          ...
          <div className="px-5 py-3">
            <div className="max-w-2xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                <button onClick={() => setShowPdfFlow(false)} className="w-8 h-8 flex items-center justify-center" style={{
                backgroundColor: colors.tile,
                borderRadius: ui.buttonRadius
              }}>
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="px-5 py-6">
            <div className="max-w-2xl mx-auto">
              <p className="leading-relaxed font-medium mb-6" style={{
              fontSize: fontSize.subtext,
              color: colors.textDark
            }}>
        ...
        <footer className="flex-shrink-0 border-t" style={{
        backgroundColor: colors.background,
        borderColor: colors.border
          ...
          <div className="px-5 py-4">
            <div className="max-w-2xl mx-auto flex items-center justify-between">
              <div />
              <button className="flex items-center justify-between gap-1.5 px-8 py-2.5 font-semibold" style={{
              backgroundColor: colors.buttonDark,
              color: colors.white,
              minWidth: 160,
              borderRadius: ui.buttonRadius,
  ...
  return <>
      ...
      `}</style>
      <LoginModal isOpen={showPlannerSaveInput} onClose={() => setShowPlannerSaveInput(false)} title="Save your plan" description="Create a free account to save your plan and track your progress." />
      ...
      height: "calc(100vh - 72px)"
            ...
            <div className="max-w-2xl mx-auto">
              ...
              padding: "60px 0 20px 0"
                ...
                gap: "12px"
                  ...
                  setMessages([{
                    type: "user",
                    content: "Yes, German or EU/EEA/Swiss"
                  }]);
        ...
        {/* Input section */}
        {!showStartScreen && <div className="flex-shrink-0 px-5 pb-1 pt-3 relative z-10" style={{
        backgroundColor: colors.background,
        borderTop: `1px solid ${colors.border}`
      }}>
            {showScrollButton && <button onClick={scrollToBottom} className="absolute z-[100] w-8 h-8 rounded-full flex items-center justify-center transition-all hover:scale-110" style={{
          backgroundColor: colors.white,
          top: "-44px",
          left: "50%",
