...
// ===========================================
const SliderInputComponent: React.FC<
  SliderInputProps & {
    label?: string;
  }
> = ({ value, onChange, onConfirm, label }) => {
...
const VisaSelectorComponent: React.FC<VisaSelectorProps> = ({ onSelect }) => {
  ...
  return (
    <div className="py-4">
      ...
      >
        Which <strong>{categoryOptions.find((c) => c.id === selectedCategory)?.label.toLowerCase()}</strong> do you
        have?
      </p>
  ...
  ({ title, content, onOpenChat, noMargin, defaultOpen = false, isLast = false }) => {
    ...
    return (
      ...
      >
        ...
        >
          <div
            className="pb-2 pl-5 leading-relaxed"
            style={{
              fontSize: fontSize.button,
              color: colors.text,
            }}
...
const ElterngeldGuide: React.FC<ElterngeldGuideProps> = ({ onOpenChat }) => {
  ...
  // ============================================
  // SCROLL UTILITIES (simplified from POC)
  // ============================================

  /**
   * Expand spacer to allow user message to scroll to top
   */
  ...
  /**
   * Adjust spacer after bot response - keeps user message at current position
   */
  const adjustSpacerAfterBotResponse = useCallback(() => {
    ...
    // Content height needed to support current scroll position
    const neededContentHeight = currentScrollTop + viewportHeight;

    // Messages height (spacer is OUTSIDE, so offsetHeight is just messages)
    const messagesHeight = messagesContainerRef.current.offsetHeight;

    // New spacer = what we need - what we have
  ...
  useEffect(() => {
    ...
    if (index >= tokens.length) {
      ...
      // After bot response: recalculate spacer and hold position
      setTimeout(() => adjustSpacerAfterBotResponse(), 50);
      callback?.();
      return;
    }
...
If your partner can't claim, you may qualify as a **single parent** and use all 14 months yourself.`,
  ...
  useEffect(() => {
    ...
    runScroll();
  }, [shouldScrollToUser, lastUserMessageIndex, expandSpacerForMessage, scrollElementToTop]);
      ...
      <div className="py-3">
        ...
        >
          ...
          >
            ...
            : +€{bonusPerMonth.toLocaleString("de-DE")}
    ...
    if (msg.type === "user") {
      ...
      return (
        <div key={i} ref={isLastUserMessage ? lastUserMessageRef : null} className="flex justify-end py-2">
          <div
            className="rounded-2xl rounded-br-sm px-4 py-2.5 max-w-[80%] bg-[#f1ede4]"
            style={{
      ...
      >
          ...
          >
            <div className="max-w-2xl mx-auto">
              {showStartScreen ? (
                ...
                <>
                  {/* Messages Container - OHNE Spacer */}
                  <div ref={messagesContainerRef}>
                    ...
                    {/* INLINE BUTTONS - vom Footer hierher verschoben */}
                  ...
                  {/* Spacer - AUSSERHALB messagesContainer */}
                  <div
                    ref={spacerRef}
                    style={{
                      height: spacerHeight,
        ...
        {!showStartScreen && (
          ...
          >
            ...
            {/* renderInput() ENTFERNT - ist jetzt inline oben */}
            <div className="max-w-2xl mx-auto">
              <div
                style={{
                  display: "flex",
